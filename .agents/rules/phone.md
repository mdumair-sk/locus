# Rule: Phone Execution & Testing

Always utilize the connected phone via ADB for:
...
4. **Native C/C++ compilation & benchmarks**: Offload to the phone via `sh ./tools/phone.sh ssh "<cmd>"` or `ssh -i ~/.ssh/id_turbotransfer -p 8022 localhost`.
