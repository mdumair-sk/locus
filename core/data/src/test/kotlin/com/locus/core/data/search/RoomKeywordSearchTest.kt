...
class RoomKeywordSearchTest {
    ...
    @Test
    fun searchAppleAgainstThreeNoteFixtureReturnsExactlyThatNote() = runTest {
        val now = Instant.ofEpochMilli(1_700_000_000_000L)
        val note1 = NoteIndexEntity(
    ...
    @Test
    fun searchWithFolderScopeFiltersCorrectly() = runTest {
        val now = Instant.ofEpochMilli(1_700_000_000_000L)
        val note1 = NoteIndexEntity(
            id = "018f1234-5678-7000-8000-000000000001",
            title = "Apple Pie",
    ...
    @Test
    fun searchWithNoteIdsScopeFiltersCorrectly() = runTest {
        val now = Instant.ofEpochMilli(1_700_000_000_000L)
        val note1 = NoteIndexEntity(
            id = "note-id-1",
            title = "Apple Juice",
    ...
    @Test
    fun searchWithTimeRangeScopeFiltersCorrectly() = runTest {
    ...
    @Test
    fun blankOrWhitespaceQueryReturnsEmptyList() = runTest {
        assertTrue(search.search("", SearchScope()).isEmpty())
        assertTrue(search.search("   ", SearchScope()).isEmpty())
    }

    @Test
    fun searchWithSpecialCharactersDoesNotCrash() = runTest {
        val now = Instant.ofEpochMilli(1_700_000_000_000L)
        val note = NoteIndexEntity(
            id = "note-special",
